...
# --- STEP 2: Reflect on goal (if recent) ---
    ...
    if goal_info.get("source") == "demo":
        ...
        st.markdown("<span style='color:#DFB743; font-size:24px'><strong>Imagine you are the following test persona:</strong></span>", unsafe_allow_html=True)
        st.markdown(f"<span style='color:#DFB743'><strong>Your name is:</strong> {nickname}</span>", unsafe_allow_html=True)
        st.markdown(f"<span style='color:#DFB743'><strong>Your most recent goal is:</strong> {goal_info.get('text', '[no goal]')}</span>", unsafe_allow_html=True)
        st.markdown(f"<span style='color:#DFB743'><strong>Some background that the system knows about you:</strong> {background}</span>", unsafe_allow_html=True)
    ...
    st.markdown(
        "<span style='color:#DFB743'><em>(For 3 or 4, use the question below to imagine what helped. For 1 or 2, imagine what got in the way.)</em></span>",
        unsafe_allow_html=True
    )
...
#  --- Chatbot motivational step---
    ...
    # --- Turn 3: Wrap up ---
        ...
        # 🟨 Feedback context
        st.markdown(
            "<span style='color:#DFB743'><strong>This was just a sample reflection, but we still want your honest feedback.</strong><br>"
